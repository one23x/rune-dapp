      ...
      <Card className="border-sky-500/20 bg-card/70 backdrop-blur">
        <CardContent className="p-4 space-y-2 text-sm">
          <div className="flex items-center gap-2 font-semibold">
            <ArrowDownToLine className="h-4 w-4 text-sky-400" /> Hyperliquid 测试网充值
          </div>
          <p className="text-xs text-muted-foreground/85 leading-relaxed">
            测试网请用官方入金口领取/充入测试 USDC:
          </p>
          <a
    ...
    <Card className="border-amber-500/20 bg-card/70 backdrop-blur">
      <CardContent className="p-4 space-y-4">
        <div className="flex items-center gap-2 text-sm font-semibold">
          <ArrowDownToLine className="h-4 w-4 text-amber-400" /> Hyperliquid 充值(非托管)
        </div>

        {/* 你的 HL 账户 = 连接的钱包 */}
        <div className="space-y-1.5">
          <div className="flex items-center gap-1.5 text-[11px] uppercase tracking-wider text-muted-foreground/70">
            <Wallet className="h-3 w-3 text-amber-300/70" /> 你的 HL 账户(= 你连接的钱包)
          </div>
          {wallet ? (
            <CopyRow label="HL 账户" value={wallet} />
          ) : (
        ...
        <ol className="space-y-2.5 text-xs text-foreground/85">
          <li className="flex gap-2">
            ...
            <span>
              在 <b>Arbitrum One</b> 上准备 <b>USDC</b>(从交易所提币到 Arbitrum,或用桥/支付服务跨链到你自己钱包)。
            </span>
          </li>
          <li className="flex gap-2">
            ...
            <span>
              从你的钱包把 USDC <b>转给 HL Bridge2 合约</b>（下方地址）。HL 会把<b>发送地址(= 你的钱包)</b>记为 HL 永续账户。
            </span>
          </li>
          <li className="flex gap-2">
            ...
            <span>约 <b>&lt;1 分钟</b>到账(最低 <b>5 USDC</b>,低于会被拒)。到账后即可开始跟单。</span>
          </li>
        </ol>
        ...
        <div className="flex gap-2 rounded-lg bg-sky-500/[0.06] border border-sky-500/20 px-3 py-2 text-[11px] text-sky-200/90 leading-relaxed">
          ...
          <span>
            这笔转账由你自己钱包发出、付一点 Arbitrum ETH 当 gas(几分钱,和在任何地方充 HL 一样)。
            <b>非托管:资金始终在你自己的 HL 账户里,平台只代下单、无法提现你的资金。</b>
          </span>
        </div>
      </CardContent>
    </Card>
