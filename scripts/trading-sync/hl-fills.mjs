...
async function getFollowers(dst) {
  const { rows } = await dst.query(`
    select distinct s.follower_address, s.network, s.user_id, u.smart_wallet_address
...
async function getFollowersDry() {
  ...
  if (process.env.FILLS_FOLLOWER) {
    return [{ follower_address: process.env.FILLS_FOLLOWER, network: process.env.FILLS_NET || "mainnet", user_id: null, smart_wallet_address: null }];
  }
  ...
  const seen = new Map();
  for (const s of subs) {
    const k = `${s.follower_address}|${s.network}`;
    if (!seen.has(k)) seen.set(k, { follower_address: s.follower_address, network: s.network, user_id: s.user_id, smart_wallet_address: null });
  }
  return [...seen.values()];
}
